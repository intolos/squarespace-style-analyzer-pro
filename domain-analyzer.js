    ...
    try {
      var result = await new Promise(function(resolve, reject) {
        var timeoutId = setTimeout(function() {
          reject(new Error('Timeout - page took longer than ' + timeoutSeconds + ' seconds to load'));
        }, currentTimeout);
              ...
              if (currentTab.status === 'complete') {
                ...
                setTimeout(async function() {
                  try {
                    ...
                    var additionalDelay = (isSquarespace && isSquarespace[0] && isSquarespace[0].result) ? 3000 : 1000;
                    ...
                    setTimeout(async function() {
                      try {
                        ...
                        if (self.options.useMobileViewport || self.options.mobileOnly) {
                          console.log('📱 DOMAIN-ANALYZER: Taking MOBILE path (direct call to MobileLighthouseAnalyzer)');
                            ...
                            if (self.options.mobileOnly) {
                              ...
                              response = {
                                ...
                                data: {
                                  themeStyles: {},
                                  siteStyles: {},
                                  buttons: { primary: { locations: [] }, secondary: { locations: [] }, tertiary: { locations: [] }, other: { locations: [] } },
                                  links: { 'in-content': { locations: [] } },
                                  images: [],
                                  colorPalette: { backgrounds: [], text: [], borders: [], all: [] },
                                  colorData: { colors: {}, contrastPairs: [] },
                                  headings: { 'heading-1': { locations: [] }, 'heading-2': { locations: [] }, 'heading-3': { locations: [] }, 'heading-4': { locations: [] } },
                                  paragraphs: { 'paragraph-1': { locations: [] }, 'paragraph-2': { locations: [] }, 'paragraph-3': { locations: [] }, 'paragraph-4': { locations: [] } },
                                  qualityChecks: { missingH1: [], multipleH1: [], brokenHeadingHierarchy: [], fontSizeInconsistency: [], missingAltText: [] },
                                  mobileIssues: {
                                    viewportMeta: MobileResultsConverter.convertViewportMeta(lighthouseResults.viewport),
                                    issues: mobileIssues
                                  },
                                  metadata: { url: pageUrl, domain: domain, pathname: pathname, title: 'Mobile Analysis', timestamp: Date.now() },
                              ...
                              if (designResponse && designResponse.success) {
                                ...
                                console.log('✅ Mobile + design analysis complete. Found', mobileIssues.length, 'mobile issues');
                              } else {
                          ...
                          console.log('💻 DOMAIN-ANALYZER: Taking DESKTOP path (analyzeStyles only)');
                          response = await chrome.tabs.sendMessage(tabId, { action: 'analyzeStyles' });
                          chrome.tabs.remove(tabId);
                        }
                    ...
                    setTimeout(async function() {
                      try {
                        ...
                        if (self.options.useMobileViewport || self.options.mobileOnly) {
                          console.log('📱 DOMAIN-ANALYZER: Taking MOBILE path (direct call - retry attempt)');
                          ...
                          console.log('💻 DOMAIN-ANALYZER: Taking DESKTOP path (analyzeStyles only - retry)');
                          response = await chrome.tabs.sendMessage(tabId, { action: 'analyzeStyles' });
                          chrome.tabs.remove(tabId);
                        }
      ...
      console.log('✅ Successfully analyzed ' + url + ' on attempt ' + (attempt + 1) + '/' + timeouts.length + ' (' + timeoutSeconds + 's timeout)');
      return result;
      
    } catch (error) {
      lastError = error;
      console.log('Attempt ' + (attempt + 1) + '/' + timeouts.length + ' failed for ' + url + ' with ' + timeoutSeconds + 's timeout: ' + error.message);
...
DomainAnalyzer.prototype.mergeAllResults = function(results) {
  ...
  console.log('🔍 MERGE: Sample result[0] structure:', results[0] ? {
    hasHeadings: !!results[0].headings,
    headingCount: results[0].headings ? Object.values(results[0].headings).reduce((sum, h) => sum + (h.locations?.length || 0), 0) : 0,
    hasParagraphs: !!results[0].paragraphs,
    paragraphCount: results[0].paragraphs ? Object.values(results[0].paragraphs).reduce((sum, p) => sum + (p.locations?.length || 0), 0) : 0,
    hasButtons: !!results[0].buttons,
    buttonCount: results[0].buttons ? Object.values(results[0].buttons).reduce((sum, b) => sum + (b.locations?.length || 0), 0) : 0,
    hasMobileIssues: !!results[0].mobileIssues,
    mobileIssueCount: results[0].mobileIssues?.issues?.length || 0
  } : 'no results');
  
  var validResults = results.filter(function(r) { return r && r.metadata; });
    ...
    try {
      ...
      if (result.siteStyles) {
        for (var style in result.siteStyles) {
          ...
          if (!merged.siteStyles[style]) {
            ...
            merged.siteStyles[style].locations = 
              merged.siteStyles[style].locations.concat(styleObj.locations);
          }
        }
      }
      ...
      if (result.buttons) {
        for (var btnType in result.buttons) {
          ...
          merged.buttons[btnType].locations = 
            merged.buttons[btnType].locations.concat(btnObj.locations);
        }
      }
      ...
      if (result.headings) {
        for (var headingType in result.headings) {
          ...
          merged.headings[headingType].locations = 
            merged.headings[headingType].locations.concat(headingObj.locations);
        }
      }
      ...
      if (result.paragraphs) {
        for (var paragraphType in result.paragraphs) {
          ...
          merged.paragraphs[paragraphType].locations = 
            merged.paragraphs[paragraphType].locations.concat(paraObj.locations);
        }
      }
      ...
      if (result.colorData) {
          ...
          for (var hex in result.colorData.colors) {
            ...
            if (!merged.colorData.colors[hex]) {
              ...
              if (colorInfo.instances) {
                merged.colorData.colors[hex].instances = 
                  merged.colorData.colors[hex].instances.concat(colorInfo.instances);
              }
            }
          }
        }
        ...
        if (result.colorData.contrastPairs && result.colorData.contrastPairs.length > 0) {
          merged.colorData.contrastPairs = 
            merged.colorData.contrastPairs.concat(result.colorData.contrastPairs);
        }
      }
      ...
      for (var check in result.qualityChecks) {
        ...
        merged.qualityChecks[check] = 
          merged.qualityChecks[check].concat(checkData);
      }
      ...
      for (var i = 0; i < resultImages.length; i++) {
        ...
        for (var j = 0; j < merged.images.length; j++) {
          if (merged.images[j].src === newImg.src && 
        ...
        if (existingIndex === -1) {
          ...
          var bothMissingAlt = (!existing.alt || existing.alt === '(missing alt text)') && 
      ...
      if (result.links && result.links['in-content']) {
        ...
        merged.links['in-content'].locations = 
          merged.links['in-content'].locations.concat(result.links['in-content'].locations);
      }
      ...
      if (result.mobileIssues) {
        ...
        if (result.mobileIssues.issues) {
          merged.mobileIssues.issues = merged.mobileIssues.issues.concat(result.mobileIssues.issues);
        }
      }
    } catch (error) {
