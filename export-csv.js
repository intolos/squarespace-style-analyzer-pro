...
// Handles exporting analysis data to CSV format

const ExportCSV = {

  // Helper function to extract font size from style definition
  ...
  // Calculate hierarchical font size issues
  calculateHierarchicalSizeIssues: function(data) {
    ...
    for (const comparison of headingComparisons) {
      ...
      if (higherSize && lowerSize) {
        if (lowerSize > higherSize) {
          const issueText = comparison.lowerLabel + ' headings (' + lowerSize + 'px most common) are larger than ' + 
                           comparison.higherLabel + ' headings (' + higherSize + 'px most common)';
          
          if (!data.qualityChecks) data.qualityChecks = {};
          if (!data.qualityChecks.fontSizeInconsistency) data.qualityChecks.fontSizeInconsistency = [];
          
          // Check if this issue already exists to avoid duplicates
          const exists = data.qualityChecks.fontSizeInconsistency.some(item => 
            item.issue === issueText || 
            (item.higherLevel === comparison.higherLabel && item.lowerLevel === comparison.lowerLabel)
          );
          ...
          if (!exists) {
            data.qualityChecks.fontSizeInconsistency.push({
              ...
              page: 'All Pages',
              issue: issueText,
              description: comparison.higherLabel + ' should be larger than ' + comparison.lowerLabel + 
                           '. Currently ' + comparison.higherLabel + ' is ' + higherSize + 'px but ' + 
                           comparison.lowerLabel + ' is ' + lowerSize + 'px.',
  ...
  export: function(accumulatedResults, filenameBrand, showSuccess, showError) {
    ...
    this.calculateHierarchicalSizeIssues(accumulatedResults);
    ...
    for (const [headingType, data] of Object.entries(accumulatedResults.headings)) {
    ...
    for (const [paragraphType, data] of Object.entries(accumulatedResults.paragraphs)) {
      for (const loc of data.locations) {
        rows.push({
          url: loc.url || '',
          navigationName: loc.navigationName || '',
    ...
    for (const [buttonType, data] of Object.entries(accumulatedResults.buttons)) {
      for (const loc of data.locations) {
        rows.push({
          url: loc.url || '',
          navigationName: loc.navigationName || '',
    ...
    if (accumulatedResults.qualityChecks) {
      for (const [checkType, issues] of Object.entries(accumulatedResults.qualityChecks)) {
        for (const issue of issues) {
          ...
          if (checkType === 'missingH1') {
            ...
            errorDescription = `Multiple H1 headings (${issue.count || 'unknown'} found). See Website Analysis report for details.`;
          } else if (checkType === 'brokenHeadingHierarchy') {
            category = 'broken Heading Hierarchy';
            errorDescription = (issue.issue || 'Broken heading hierarchy') + '. See Website Analysis report for details.';
          } else if (checkType === 'fontSizeInconsistency') {
            errorDescription = (issue.issue || issue.description || 'Font size inconsistency') + '. See Website Analysis report for details.';
          } else if (checkType === 'missingAltText') {
            category = 'missing Alt Text on image';
            errorDescription = 'See downloaded Images Analysis report in the Missing Alt Text section for image links.';
          } else if (checkType === 'genericImageNames') {
            category = 'generic Image Names';
            errorDescription = 'See downloaded Images Analysis report in the Generic Image Filenames section for image links.';
          } else if (checkType === 'styleInconsistency') {
            errorDescription = (issue.issue || issue.description || 'Style inconsistency detected') + '. See Website Analysis report for details.';
          } else {
            // More descriptive fallback
            const typeName = checkType.replace(/([A-Z])/g, ' $1').trim();
            errorDescription = (issue.issue || issue.description || issue.text || typeName) + '. See Website Analysis report for details.';
          }
