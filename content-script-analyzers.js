...
var ContentScriptAnalyzers = (function() {
  ...
  function analyzeButtons(results, navigationName, colorTracker, colorData) {
    var buttonSelectors = [
      ...
      'a.button', 'a.btn',
      'a[class*="sqs-button"]', 'a[class*="sqs-block-button"]',
      '.sqs-block-button-element',
      'a[href][class*="btn"]', 'a[href][class*="button"]',
      'input[type="submit"]', 'input[type="button"]',
      '.button-block a', 'a[role="button"]',
    ...
    var excludedPatterns = ['open menu', 'skip to content', 'skip to', 'close menu', 'folder:', 'cookie', 'large images', 'all images', 'images (>100kb)', 'pause background', 'play background', 'background'];
    ...
    for (var i = 0; i < buttons.length; i++) {
      ...
      if (btnClasses.includes('accordion') || btnClasses.includes('collapse') || btnClasses.includes('toggle') || btnClasses.includes('dropdown')) isAccordion = true;
      
      var parentEl = btn;
      for (var p = 0; p < 3; p++) {
        if (!parentEl) break;
        var parentClass = (parentEl.className || '').toLowerCase();
        if (parentClass.includes('accordion') || parentClass.includes('faq') || parentClass.includes('collapse')) {
          isAccordion = true;
          break;
        }
  ...
  function analyzeHeadings(results, navigationName, colorTracker, colorData) {
    ...
    if (headingSequence.length > 0) {
      ...
      for (var i = 0; i < headings.length; i++) {
        ...
        if (lastLevel > 0 && currentLevel > lastLevel + 1) {
          ...
          var issueText = 'Broken hierarchy: H' + lastLevel + ' -> H' + currentLevel + ' (skipped ' + skippedLevels.join(', ') + ')';
          var description = 'After "' + lastHeadingText.substring(0, 50) + (lastHeadingText.length > 50 ? '...' : '') + '" (H' + lastLevel + '), jumped to "' + currentText.substring(0, 50) + (currentText.length > 50 ? '...' : '') + '" (H' + currentLevel + ') in ' + section + ' / ' + block;
    ...
    var headingTypes = ['heading-1', 'heading-2', 'heading-3', 'heading-4', 'heading-5', 'heading-6'];
    for (var ht = 0; ht < headingTypes.length; ht++) {
      ...
      if (uniqueSizes.length >= 3) {
        var distributionText = uniqueSizes.map(function(size) {
          return size + ' (' + sizesMap[size].count + ' times)';
        }).join(', ');
        
        var headingLabel = headingType.replace('heading-', 'H');
        var issueText = headingType.replace('-', ' ').toUpperCase() + ' appears in ' + uniqueSizes.length + ' different font sizes';
        var description = headingLabel + ' headings found in ' + uniqueSizes.length + ' different sizes: ' + distributionText + '. Review if these variations are intentional.';
  ...
  function analyzeParagraphs(results, navigationName, squarespaceThemeStyles, colorTracker, colorData) {
    ...
    for (var j = 0; j < paragraphs.length; j++) {
      ...
      if (text.length > 10) {
        ...
        if (pClasses.includes('paragraph-1') || pClasses.includes('intro-text') || pClasses.includes('hero-text') || pClasses.includes('lead')) {
          paragraphType = 'paragraph-1';
        } else if (pClasses.includes('paragraph-3') || pClasses.includes('caption') || pClasses.includes('quote') || pClasses.includes('blockquote')) {
          ...
          while (parentEl && depth < 5) {
            ...
            if (parentClass.includes('hero') || parentClass.includes('banner') || parentClass.includes('intro')) {
              paragraphType = 'paragraph-1';
              break;
            } else if (parentClass.includes('footer') || parentClass.includes('sidebar')) {
              paragraphType = 'paragraph-4';
              break;
            } else if (parentClass.includes('testimonial') || parentClass.includes('quote') || parentClass.includes('blockquote')) {
              paragraphType = 'paragraph-3';
              break;
            }
  ...
  function analyzeLinks(results, navigationName, colorTracker, colorData) {
    ...
    var excludeSelectors = [
      ...
      '.navigation a', '.menu a',
      '[class*="nav"] a', '[class*="footer"] a',
      '[class*="social"] a', '[class*="share"] a',
      'a.button', 'a.btn',
      'a[class*="sqs-button"]', 'a[class*="btn"]',
      'a[role="button"]', '.button-block a'
    ];
    ...
    for (var ca = 0; ca < contentAreas.length; ca++) {
      ...
      for (var li = 0; li < allLinks.length; li++) {
        ...
        for (var ex = 0; ex < excludeSelectors.length; ex++) {
          if (link.matches(excludeSelectors[ex]) || link.closest(excludeSelectors[ex].replace(' a', ''))) {
            shouldExclude = true;
            break;
          }
        }
        ...
        results.links['in-content'].locations.push({
          ...
          url: window.location.href,
          text: text.substring(0, 200),
          href: href.length > 150 ? href.substring(0, 150) + '...' : href,
          styleDefinition: styleDefinition,
          section: section,
          block: block,
          pageTitle: document.title || 'Unknown'
  ...
  function isGenericImageFilename(filename) {
    ...
    if (/^(DSC|DSCN|DSCF|IMG|DCIM|SAM|IMAG|PIC|PHOTO|LG|PXL|GOPR|DJI|MVIMG|VID|MOV|MVI|PANO|BURST|HDR|PORTRAIT)[-_]?\d+/i.test(filename)) return 'camera-default';
    ...
    // Stock photo sites
    if (/^(shutterstock|istock|istockphoto|pexels|adobe[-_]?stock|dreamstime|depositphotos|gettyimages|getty[-_]?images|stock[-_]?photo|unsplash|pixabay|freepik|vecteezy|rawpixel|stocksy|alamy|123rf|bigstock|canstock|fotolia|thinkstock|corbis|envato|elements[-_]?envato|pond5|videvo|storyblocks|motion[-_]?array|twenty20|eyeem|500px|flickr[-_]?cc|creative[-_]?commons)/i.test(filename)) return 'stock-photo';
    
    // Generic placeholder names
    if (/^(untitled|picture|photo|image|imagen|foto|unnamed|unknown|default|placeholder|temp|test|sample|example|demo|dummy|filler|blank|empty|new[-_]?image|copy[-_]?of)\d*\./i.test(lowerFilename)) return 'generic-name';
    
    // Screenshots
    if (/^(screenshot|screen[-_]?shot|capture|screen[-_]?cap|screen[-_]?grab|snip|snipping)/i.test(filename)) return 'screenshot';
    if (/^(Screenshot|Captura|Bildschirmfoto|Schermafbeelding|Skjermbilde)[-_\s]/i.test(filename)) return 'screenshot'; // Localized screenshot names
    ...
    // Full UUID
    if (/^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/i.test(nameWithoutExt)) return 'uuid';
...
function isLikelyIcon(img, src, imgWidth, imgHeight) {
    ...
    if (fullPath.includes('/icons/') || fullPath.includes('/assets/icons/') ||
        fullPath.includes('/images/icons/') || fullPath.includes('/img/icons/')) {
      return true;
    }
    ...
    // Check 4: Very small dimensions (≤32x32) - likely decorative icons
    // Increased threshold from 64x64 to be more conservative
    if ((imgWidth > 0 && imgWidth <= 32) && (imgHeight > 0 && imgHeight <= 32)) {
      return true;
    }

    // Check 5: SVG icons with small dimensions and icon-related paths/classes
    if (fullPath.endsWith('.svg') && (imgWidth <= 32 || imgHeight <= 32)) {
      return true;
    }
