...
const ResultsManager = {
  ...
  mergeResults: function(accumulatedResults, newResults) {
    ...
    if (newResults.siteStyles) {
      for (const style in newResults.siteStyles) {
        ...
        if (!accumulatedResults.siteStyles[style]) {
          accumulatedResults.siteStyles[style] = { ...styleObj, locations: [...styleObj.locations] };
        } else {
          if (!accumulatedResults.siteStyles[style].locations) {
            accumulatedResults.siteStyles[style].locations = [];
          }
          accumulatedResults.siteStyles[style].locations = 
            accumulatedResults.siteStyles[style].locations.concat(styleObj.locations);
        }
      }
    }
    ...
    if (newResults.buttons) {
      for (const btnType in newResults.buttons) {
        ...
        if (!accumulatedResults.buttons[btnType]) {
          ...
          accumulatedResults.buttons[btnType].locations = 
            accumulatedResults.buttons[btnType].locations.concat(btnObj.locations);
        }
      }
    }
    ...
    if (newResults.links && newResults.links['in-content']) {
      ...
      accumulatedResults.links['in-content'].locations = 
        accumulatedResults.links['in-content'].locations.concat(newResults.links['in-content'].locations);
    }

// Merge mobile issues
    if (!accumulatedResults.mobileIssues) {
      accumulatedResults.mobileIssues = { viewportMeta: { exists: false, content: null, isProper: false }, issues: [] };
    }
    if (newResults.mobileIssues) {
      ...
      if (newResults.mobileIssues.issues) {
        accumulatedResults.mobileIssues.issues = accumulatedResults.mobileIssues.issues.concat(newResults.mobileIssues.issues);
      }
    }
    ...
    if (!accumulatedResults.colorPalette) {
      ...
      const allColors = new Set(accumulatedResults.colorPalette.all.concat(newResults.colorPalette.all));
      const bgColors = new Set(accumulatedResults.colorPalette.backgrounds.concat(newResults.colorPalette.backgrounds));
      const textColors = new Set(accumulatedResults.colorPalette.text.concat(newResults.colorPalette.text));
      const borderColors = new Set(accumulatedResults.colorPalette.borders.concat(newResults.colorPalette.borders));
    ...
    if (newResults.headings) {
      for (const headingType in newResults.headings) {
        ...
        accumulatedResults.headings[headingType].locations = 
          accumulatedResults.headings[headingType].locations.concat(headingObj.locations);
      }
    }
    ...
    if (newResults.paragraphs) {
      for (const paragraphType in newResults.paragraphs) {
        ...
        accumulatedResults.paragraphs[paragraphType].locations = 
          accumulatedResults.paragraphs[paragraphType].locations.concat(paraObj.locations);
      }
    }
  ...
  resetAnalysis: async function(showSuccessCallback, hideMessagesCallback) {
    const confirmed = await customConfirm('This will clear all accumulated analysis data from multiple pages. Are you sure?');
  ...
  hasResults: function(accumulatedResults) {
    return accumulatedResults !== null && 
  ...
  isPageAnalyzed: function(accumulatedResults, pathname) {
    if (!accumulatedResults || !accumulatedResults.metadata || !accumulatedResults.metadata.pagesAnalyzed) {
      return false;
    }
