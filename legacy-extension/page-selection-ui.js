...
const PageSelectionUI = {
  ...
  populateNavigationSections: function (groupedUrls) {
    ...
    for (const key in groupedUrls.sections) {
      ...
      if (childCount > 0 && childCount <= 6) {
        if (DEBUG_PSU) console.log('DEBUG: Creating section WITH children for:', section.name);
        // Show parent with individual child checkboxes
        this.createSectionWithChildren(container, section, sectionIndex++);
      } else if (childCount > 6) {
        if (DEBUG_PSU) console.log('DEBUG: Creating section WITH subgroup for:', section.name);
        // Show parent, then subgroup for children
        this.createSectionWithSubgroup(container, section, sectionIndex++);
      } else {
        if (DEBUG_PSU) console.log('DEBUG: Creating regular section for:', section.name);
    ...
    if (groupedUrls.otherNavLinks && groupedUrls.otherNavLinks.count > 0) {
      if (DEBUG_PSU) console.log('DEBUG: Creating otherNavLinks section');
      this.createSectionCheckbox(
        container,
        groupedUrls.otherNavLinks,
        sectionIndex++,
        'otherNavLinks'
      );
    }
    ...
    if (groupedUrls.blog && groupedUrls.blog.count > 0) {
      if (DEBUG_PSU) console.log('DEBUG: Creating blog section');
    ...
    if (groupedUrls.other && groupedUrls.other.count > 0) {
      if (DEBUG_PSU) console.log('DEBUG: Creating other section checkbox');
  ...
  createSectionCheckbox: function (container, section, index, type) {
    ...
    `;
      ...
      console.log(
        'DEBUG: Appended item to container for:',
        section.name,
        'container children:',
  ...
  handleNavigationSelection: function (useMobileViewport, mobileOnly) {
    ...
    if (DEBUG_PSU) console.log('🎯 Selected URLs for analysis:', urlsToAnalyze.length);
