...
async function captureScreenshotWithRetry(maxRetries: number = 3): Promise<string | null> {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.warn(
        `[SSA] Screenshot attempt ${attempt} failed:`,
 ...
 */
function analyzeCSSForBackgroundOriginal(element: Element): {
  color: string | null;
  details: string;
} {
  ...
  const backgroundClasses = classList.filter(
    cls => cls.includes('background') || cls.includes('lp-background') || cls.includes('bg')
  );
...
async function sampleCanvasColors(
  ...
  samples: Array<{ x: number; y: number; color: string }>;
}> {
  if (!screenshot) {
 ...
 * Expected time: ~25ms
 */
async function smartHybridMethod(
  element: Element,
  screenshot: string | null
 ...
 */
async function fullHybridMethod(
  element: Element,
  screenshot: string | null
): Promise<MethodResult> {
  const startTime = performance.now();
  ...
  if (canvasResult.consistency > 70 && canvasResult.dominantColor) {
    ...
    if (
      cssResult.color &&
      cssResult.color.toLowerCase() !== canvasResult.dominantColor.toLowerCase()
    ) {
 ...
 * 7. indeterminate
 */
async function sqsCurrentMethod(
  element: Element,
  screenshot: string | null,
  ...
  if (domResult.color) {
    ...
    if (SUSPICIOUS_COLORS.has(domResult.color.toUpperCase())) {
      ...
      if (
        canvasVerify.canvasColor &&
        canvasVerify.confidence === 'high' &&
        canvasVerify.canvasColor.toLowerCase() !== domResult.color.toLowerCase()
      ) {
        return {
...
async function sqsProposedMethod(element: Element): Promise<MethodResult> {
  ...
  if (lastClickCoordinates) {
    ...
    console.log(
      '[SQS Debug] elementFromPoint returned:',
      elementAtPoint ? elementAtPoint.tagName : 'null'
    );

    if (elementAtPoint) {
      console.log(
        '[SQS Debug] Element at point:',
        elementAtPoint.tagName,
        elementAtPoint.className?.substring(0, 30)
      );
      ...
      if (sectionElement) {
        console.log(
          '[SQS Debug] Section found:',
          sectionElement.tagName,
          'class:',
          sectionElement.className?.substring(0, 30)
        );
      }
    } else {
    ...
    if (!sectionElement) {
      ...
      console.log(
        '[SQS Debug] Checking all sections by bounding rect... Total:',
        allSections.length
      );
      ...
      for (let i = 0; i < allSections.length; i++) {
        ...
        const isInRect =
          lastClickCoordinates.y >= rect.top &&
          lastClickCoordinates.y <= rect.bottom &&
          lastClickCoordinates.x >= rect.left &&
          lastClickCoordinates.x <= rect.right;
        console.log(
          `[SQS Debug] Section ${i}: top=${rect.top}, bottom=${rect.bottom}, inRect=${isInRect}`
        );
        if (isInRect) {
...
async function runTestOnElement(element: Element, screenshot: string | null): Promise<TestResult> {
  ...
  const [fastPathOriginal, fastPath, smartHybrid, fullHybrid, sqsCurrent, sqsProposed] =
    await Promise.all([
...
function createTestOverlay(result: TestResult): HTMLElement {
        ...
        <tr style="border-bottom: 1px solid #ddd; background: #e8f4e8;">
          ...
          <td style="padding: 8px;">
            ${
              ...
              ${result.methods.sqsCurrent?.color}
            `
                : result.methods.sqsCurrent?.details?.includes('Indeterminate')
                  ? `<span style="color: #666; font-style: italic;">Indeterminate</span>`
                  : 'N/A'
            }
          </td>
        ...
        <tr style="background: #e8f4e8;">
          ...
          <td style="padding: 8px;">
            ${
              ...
              ${result.methods.sqsProposed?.color}
            `
                : result.methods.sqsProposed?.details?.includes('Indeterminate')
                  ? `<span style="color: #666; font-style: italic;">Indeterminate</span>`
                  : 'N/A'
            }
          </td>
  ...
  const header = overlay.querySelector('#color-test-header') as HTMLElement;
  if (header) {
...
export async function exportTestResults(): Promise<string> {
  ...
  const validResults = testResults.filter(
    r => r.methods && r.methods.sqsCurrent && r.methods.sqsProposed
  );
  ...
  const rows = resultsToExport.map(result => {
    ...
    const fastOriginalAccurate =
      manual && result.methods.fastPathOriginal?.color?.toLowerCase() === manual ? 'YES' : 'NO';
    const fastAccurate =
      manual && result.methods.fastPath?.color?.toLowerCase() === manual ? 'YES' : 'NO';
    const smartAccurate =
      manual && result.methods.smartHybrid?.color?.toLowerCase() === manual ? 'YES' : 'NO';
    const fullAccurate =
      manual && result.methods.fullHybrid?.color?.toLowerCase() === manual ? 'YES' : 'NO';
    const sqsCurrentAccurate =
      manual && result.methods.sqsCurrent?.color?.toLowerCase() === manual ? 'YES' : 'NO';
    const sqsProposedAccurate =
      manual && result.methods.sqsProposed?.color?.toLowerCase() === manual ? 'YES' : 'NO';

    // Calculate 1diff1 (1 difference by 1) - with null checks
    const fastOriginal1diff1 = calculateOneDiffOne(
      result.manualVerification,
      result.methods.fastPathOriginal?.color
    );
    const fast1diff1 = calculateOneDiffOne(
      result.manualVerification,
      result.methods.fastPath?.color
    );
    const smart1diff1 = calculateOneDiffOne(
      result.manualVerification,
      result.methods.smartHybrid?.color
    );
    const full1diff1 = calculateOneDiffOne(
      result.manualVerification,
      result.methods.fullHybrid?.color
    );
    const sqsCurrent1diff1 = calculateOneDiffOne(
      result.manualVerification,
      result.methods.sqsCurrent?.color
    );
    const sqsProposed1diff1 = calculateOneDiffOne(
      result.manualVerification,
      result.methods.sqsProposed?.color
    );
    ...
    if (fastOriginalAccurate === 'YES')
      accurateMethods.push({
        name: 'Fast_Path_Original',
        time: result.methods.fastPathOriginal?.timeMs ?? 0,
      });
    if (fastAccurate === 'YES')
      accurateMethods.push({ name: 'Fast_Path', time: result.methods.fastPath?.timeMs ?? 0 });
    if (smartAccurate === 'YES')
      accurateMethods.push({ name: 'Smart_Hybrid', time: result.methods.smartHybrid?.timeMs ?? 0 });
    if (fullAccurate === 'YES')
      accurateMethods.push({ name: 'Full_Hybrid', time: result.methods.fullHybrid?.timeMs ?? 0 });
    if (sqsCurrentAccurate === 'YES')
      accurateMethods.push({ name: 'SQS_Current', time: result.methods.sqsCurrent?.timeMs ?? 0 });
    if (sqsProposedAccurate === 'YES')
      accurateMethods.push({ name: 'SQS_Proposed', time: result.methods.sqsProposed?.timeMs ?? 0 });
...
export function getTestStats(): object {
  ...
  const avgTimeFastOriginal =
    validResults.reduce((sum, r) => sum + (r.methods.fastPathOriginal?.timeMs ?? 0), 0) /
    validResults.length;
  const avgTimeFast =
    validResults.reduce((sum, r) => sum + (r.methods.fastPath?.timeMs ?? 0), 0) /
    validResults.length;
  const avgTimeSmart =
    validResults.reduce((sum, r) => sum + (r.methods.smartHybrid?.timeMs ?? 0), 0) /
    validResults.length;
  const avgTimeFull =
    validResults.reduce((sum, r) => sum + (r.methods.fullHybrid?.timeMs ?? 0), 0) /
    validResults.length;
  const avgTimeSqsCurrent =
    validResults.reduce((sum, r) => sum + (r.methods.sqsCurrent?.timeMs ?? 0), 0) /
    validResults.length;
  const avgTimeSqsProposed =
    validResults.reduce((sum, r) => sum + (r.methods.sqsProposed?.timeMs ?? 0), 0) /
    validResults.length;
...
export async function addTestCase(
  ...
  if (expectedColor) {
    ...
    console.log(
      `[SSA Test] ${match ? '✅ PASS' : '❌ FAIL'} - Detected: ${detectedColor}, Expected: ${expectedColor}`
    );
  }
  ...
  const testCases = [
    {
      selector: 'h1, h2, h3',
      label: 'sqs-heading',
      description: 'Headings (common contrast issues)',
    },
    {
      selector: 'p',
      label: 'sqs-paragraph',
      description: 'Paragraphs (text on section backgrounds)',
    },
    {
      selector: '.sqs-block-button-element, .btn, .button',
      label: 'sqs-button',
      description: 'Buttons',
    },
