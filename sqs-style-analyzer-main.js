...
(function() {
        ...
        try {
          ...
          for (let i = 0; i < issues.length; i++) {
            ...
            try {
              ...
              const elementThumbnail = await captureElementScreenshot(fullPageScreenshot, element, 20);
              const elementContext = await captureElementScreenshot(fullPageScreenshot, element, 200);
  ...
  async function captureElementScreenshot(fullScreenshot, element, paddingPx = 20) {
    ...
    try {
      ...
      const isOutsideViewport = rect.top < 0 || rect.top > window.innerHeight || rect.bottom < 0 || rect.bottom > window.innerHeight;
  ...
  async function analyzeSquarespaceStyles() {
    ...
    try {
      ...
      for (const violation of axeResults.violations) {
        for (const node of violation.nodes) {
          ...
          let elementText = element.textContent.trim().substring(0, 100) ||
                           element.getAttribute('aria-label') ||
                           element.getAttribute('title') || 'No text';
          ...
          if (fullPageScreenshot) {
            ...
            console.log('Screenshots captured:', elementText.substring(0, 30),
                       'Thumbnail:', !!elementThumbnail, 'Context:', !!elementContext);
          }
    ...
    try {
      await ContentScriptAnalyzers.analyzeButtons(results, navigationName, colorTracker, results.colorData);
    ...
    try {
      await ContentScriptAnalyzers.analyzeHeadings(results, navigationName, colorTracker, results.colorData);
    ...
    try {
      await ContentScriptAnalyzers.analyzeParagraphs(results, navigationName, squarespaceThemeStyles, colorTracker, results.colorData);
    ...
    try {
      await ContentScriptAnalyzers.analyzeLinks(results, navigationName, colorTracker, results.colorData);
